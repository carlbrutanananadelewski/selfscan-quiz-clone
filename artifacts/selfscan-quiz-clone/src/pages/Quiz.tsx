...
export function Quiz() {
  ...
  const progress = Math.round((currentIndex / (QUIZ_QUESTIONS.length - 1)) * 100);
  ...
  const canAdvance = () => {
    ...
    if (question.type === 'lead-capture') {
      const form = currentAnswer as any;
  ...
  const renderOptions = () => {
    ...
    if (question.type === 'lead-capture') {
      const form = (currentAnswer as any) || { name: '', email: '' };
      return (
    ...
    if (question.type === 'recommendation') {
      const userName = (answers['q12'] as any)?.name || 'there';
          ...
          {question.id === 'q1' ? (
            <div className="flex flex-col md:flex-row items-center justify-between gap-8 md:gap-24">
              ...
              <div className="flex-1 max-w-[550px] w-full text-center md:text-left">
                ...
                <h1 className="text-xl md:text-2xl font-bold mb-2">
                  {question.title.replace('{name}', (answers['q12'] as any)?.name || 'there')}
                </h1>
                {question.subtitle && (
            ...
            <div className="text-center w-full">
              <h1 className="text-2xl md:text-3xl font-bold mb-3 font-serif max-w-3xl mx-auto leading-tight">
                {question.title.replace('{name}', (answers['q12'] as any)?.name?.split(' ')[0] || '')}
              </h1>
              {question.subtitle && (
        ...
        <div className="flex justify-between items-center mb-3">
          ...
          <div className="flex gap-2">
            ...
            {!isLast && (
              ...
              >
                {/* Single-select auto-advances, but button is still there. Multi-select requires button. */}
                <span className="font-sans font-medium text-sm">Next</span>
              </button>
            )}
          </div>
        </div>
